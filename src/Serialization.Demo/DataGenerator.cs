...
namespace Serialization.Demo
{
	...
	{
		private static readonly Dictionary<Type, Func<Faker, IPet>> petBuilder = new Dictionary<Type, Func<Faker, IPet>>() { { typeof (Cat), (f) => BuildACat (f) }, { typeof (Dog), (f) => BuildADog (f) }, { typeof (GoldFish), (f) => BuildAGoldFish (f) }, { typeof (Timisoreana), (f) => BuildATimisoreana (f) },
		};
		...
		public static BFO GetBFO()
		{
			List<Data.Person> persons = new List<Data.Person>();

			var customFaker = HouseFaker()
				.FinishWith((f, h) =>
				{
					persons.AddRange(h.Habitants);
				});
			...
			var bfoFaker = new Faker<BFO>()
				...
				.RuleFor(p => p.Houses, f => customFaker.Generate(30));

			return bfoFaker.Generate();
		}

		private static Faker<House> HouseFaker()
		{
		...
		public static Faker<Data.Person> PersonsFaker()
		{
			return new Faker<Data.Person>()
		...
		public static Faker<IPet> PetsFaker()
		{
